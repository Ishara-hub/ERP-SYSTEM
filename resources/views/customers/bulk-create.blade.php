...
<div class="bg-gray-50 fixed-page-container">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-4 py-6 fixed-page-wrapper">
        ...
        <div class="flex justify-between items-center mb-6 page-header-fixed">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Bulk Create Customers - Spreadsheet View</h1>
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('customers.web.index') }}" class="inline-flex items-center px-3 py-2 bg-gray-200 text-gray-800 text-sm rounded-md hover:bg-gray-300">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form method="POST" action="{{ route('customers.web.bulk-store') }}" id="bulkCustomersForm">
            @csrf
                    ...
                    <table class="min-w-full border-collapse" id="spreadsheetTable" style="font-family: 'Courier New', monospace;">
                        <thead class="bg-gray-100 sticky top-0 z-10">
                            <tr>
                                ...
                                <th class="min-w-[200px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">B</span> Email
                                </th>
                                <th class="min-w-[150px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">C</span> Phone
                                </th>
                                <th class="min-w-[250px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">D</span> Address
                                </th>
                                <th class="min-w-[180px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">E</span> Company
                                </th>
                                <th class="min-w-[180px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">F</span> Contact Person
                                </th>
                                <th class="min-w-[200px] px-2 py-2 text-left text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">G</span> Notes
                                </th>
                                <th class="min-w-[100px] px-2 py-2 text-center text-xs font-semibold text-gray-600 border border-gray-300 bg-gray-200">
                                    <span class="text-blue-600">H</span> Active
                                </th>
                            </tr>
                        </thead>
                        <tbody id="customersBody" class="bg-white">
                            @for($i = 0; $i < 20; $i++)
                            <tr class="customer-row hover:bg-blue-50">
                                ...
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][name]" type="text" 
                                ...
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][email]" type="email" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="email@example.com" 
                                           data-col="1" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][phone]" type="text" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="Phone" 
                                           data-col="2" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][address]" type="text" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="Address" 
                                           data-col="3" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][company]" type="text" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="Company" 
                                           data-col="4" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][contact_person]" type="text" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="Contact Person" 
                                           data-col="5" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300">
                                    <input name="customers[{{ $i }}][notes]" type="text" 
                                           class="w-full px-2 py-1.5 border-0 outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 bg-transparent text-sm" 
                                           placeholder="Notes" 
                                           data-col="6" data-row="{{ $i }}" />
                                </td>
                                <td class="px-1 py-0 border border-gray-300 text-center">
                                    <input type="hidden" name="customers[{{ $i }}][is_active]" value="0" />
                                    <input type="checkbox" name="customers[{{ $i }}][is_active]" value="1" 
...
<style>
    ...
    #bulkCustomersForm > div.bg-white {
        flex: 1;
        display: flex;
        flex-direction: column;
        overflow: hidden;
    ...
    #bulkCustomersForm > div.bg-white > div.overflow-x-auto {
        flex: 1;
        overflow-x: auto;
        overflow-y: auto;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handlePaste(e) {
        ...
        rows.forEach((row, rowOffset) => {
            ...
            cols.forEach((value, colOffset) => {
                ...
                if (targetCol > 7) return; // Max 8 columns (0-7)

                const rowEl = body.querySelector(`tr[data-row-index="${targetRow}"]`) || 
                             Array.from(body.querySelectorAll('tr.customer-row'))[targetRow];
    ...
    function navigateToCell(row, col, direction = 'right') {
        const rows = Array.from(body.querySelectorAll('tr.customer-row'));
    ...
    body.addEventListener('focusout', (e) => {
        if (e.target.closest('tr')) {
            ...
            const rowIndex = Array.from(body.querySelectorAll('tr.customer-row')).indexOf(row);
    ...
    function addRows(count = 10) {
        const existingRows = body.querySelectorAll('tr.customer-row').length;
        const baseRow = body.querySelector('tr.customer-row');
    ...
    function updateRowCount() {
        const count = body.querySelectorAll('tr.customer-row').length;
        rowCountEl.textContent = count;
    }

    // Add row index attributes
    body.querySelectorAll('tr.customer-row').forEach((row, idx) => {
    ...
    document.addEventListener('keydown', (e) => {
        if ((e.ctrlKey || e.metaKey) && e.key === 's') {
            e.preventDefault();
            document.getElementById('bulkCustomersForm').submit();
        }
    });
});
</script>
