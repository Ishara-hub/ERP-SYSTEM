    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Sales by Item Report</h1>
                    <p class="mt-1 text-sm text-gray-600">View sales performance grouped by product</p>
                </div>
        ...
        <div class="bg-white shadow-sm rounded-lg mb-6">
            <form method="GET" action="{{ route('reports.sales-by-item') }}" class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="item_id" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">{{ $itemSales->count() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">{{ number_format($itemSales->sum('total_quantity_sold'), 2) }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">${{ number_format($itemSales->sum('total_revenue'), 2) }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($itemSales as $sales)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-medium text-gray-900">{{ $sales['item_name'] }}</div>
                                    <div class="text-sm text-gray-500">{{ $sales['item_number'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-900">
                                    {{ number_format($sales['total_quantity_sold'], 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">
                                    ${{ number_format($sales['total_revenue'], 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500">
                                    ${{ number_format($sales['average_price'], 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-900">
                                    {{ $sales['invoice_count'] }}
                                </td>
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center font-bold text-gray-900">{{ number_format($itemSales->sum('total_quantity_sold'), 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right font-bold text-gray-900">${{ number_format($itemSales->sum('total_revenue'), 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right font-bold text-gray-500">-</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center font-bold text-gray-900">{{ $itemSales->sum('invoice_count') }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
