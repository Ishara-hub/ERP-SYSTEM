...
@section('title', 'Purchases by Item')
...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Purchases by Item</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Purchases by Item Detail</h1>
        ...
        <div class="bg-white shadow-sm rounded-lg mb-6 no-print">
            <div class="p-6">
                <form method="GET" action="{{ route('reports.inventory.purchases-by-item') }}" class="flex items-end space-x-4">
                    <div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-xs">
                        <thead>
                            <tr class="bg-gray-50 text-2xs font-bold text-gray-500 uppercase tracking-wider">
                                ...
                                <th class="px-3 py-3 text-center">Qty Purchased</th>
                                <th class="px-3 py-3 text-right">Unit Cost</th>
                                <th class="px-3 py-3 text-right">Purchase Amount</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                            ...
                            @foreach($purchases as $row)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-4 whitespace-nowrap text-gray-700">#{{ $row['bill_no'] }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-gray-700">{{ $row['date']->format('Y-m-d') }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-gray-700">{{ $row['vendor'] }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-center">{{ number_format($row['qty_purchased'], 2) }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-right text-gray-900">Rs.{{ number_format($row['unit_cost'], 2) }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-right text-gray-900 font-semibold">Rs.{{ number_format($row['purchase_amount'], 2) }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-right text-gray-600">Rs.{{ number_format($row['tax'], 2) }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-right text-gray-900 font-bold">Rs.{{ number_format($row['total_amount'], 2) }}</td>
                                </tr>
                                @php 
                        ...
                        <tfoot>
                            <tr class="bg-gray-100 font-bold border-t-2 border-gray-300 uppercase tracking-wider">
                                ...
                                <td class="px-3 py-4 whitespace-nowrap text-sm text-right text-gray-900">Rs.{{ number_format($totalPurchase, 2) }}</td>
                                <td class="px-3 py-4 whitespace-nowrap text-sm text-right text-gray-600">Rs.{{ number_format($totalTax, 2) }}</td>
                                <td class="px-3 py-4 whitespace-nowrap text-sm text-right text-blue-800 font-black">Rs.{{ number_format($grandTotal, 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
