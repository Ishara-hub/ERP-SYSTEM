    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Stock Movement Report</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Full movement history from {{ \Carbon\Carbon::parse($dateFrom)->format('M d, Y') }} to {{ \Carbon\Carbon::parse($dateTo)->format('M d, Y') }}</p>
            </div>
        ...
        <div class="bg-white shadow-sm rounded-lg mb-6 no-print">
            <div class="p-6">
                <form method="GET" action="{{ route('reports.inventory.stock-movement') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-xs">
                        <thead>
                            <tr class="bg-gray-50 text-2xs font-bold text-gray-500 uppercase tracking-wider">
                                ...
                                <th class="px-3 py-3 text-center">In Qty</th>
                                <th class="px-3 py-3 text-center">Out Qty</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($movements as $m)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-3 py-4 whitespace-nowrap text-gray-900">{{ $m->transaction_date->format('Y-m-d') }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap font-medium text-gray-900">{{ optional($m->item)->item_name }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap">
                                        @php
                                            ...
                                            $typeColor = $typeColors[$m->type] ?? 'bg-gray-100 text-gray-800';
                                        @endphp
                                        <span class="px-2 py-0.5 rounded-full text-2xs font-medium {{ $typeColor }}">
                                            {{ ucfirst($m->type) }}
                                        </span>
                                    </td>
                                    <td class="px-3 py-4 whitespace-nowrap text-gray-700">#{{ $m->source_document_id }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-center text-green-600 font-semibold">{{ $m->quantity > 0 ? '+' . number_format($m->quantity, 2) : '-' }}</td>
                                    <td class="px-3 py-4 whitespace-nowrap text-center text-red-600 font-semibold">{{ $m->quantity < 0 ? number_format(abs($m->quantity), 2) : '-' }}</td>
                                    <td class="px-3 py-4 text-gray-500 italic">{{ $m->description }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
