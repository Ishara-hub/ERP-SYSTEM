...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $branches_check = $conn->query("SELECT COUNT(*) as count FROM branches");
        $branch_count = $branches_check->fetch_assoc()['count'];
        
        if ($branch_count == 0) {
            throw new Exception("No branches available. Please add branches first before creating journal entries.");
        }
        ...
        $stmt = $conn->prepare("INSERT INTO general_journal (transaction_date, reference, description, created_by, branch_id) 
                               VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssii", $_POST['transaction_date'], $_POST['reference'], $_POST['description'], $_SESSION['user_id'], $_POST['branch_id']);
        $stmt->execute();
