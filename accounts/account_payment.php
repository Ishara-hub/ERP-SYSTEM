...
function generate_transaction_id() {
    ...
    global $conn;
    $stmt = $conn->prepare("SELECT MAX(id) FROM general_journal");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if (empty($_POST['branch_id']) || !is_numeric($_POST['branch_id'])) {
            throw new Exception("Please select a valid branch.");
        }
        ...
        $branches_check = $conn->query("SELECT COUNT(*) as count FROM branches WHERE id = " . intval($_POST['branch_id']));
        $branch_exists = $branches_check->fetch_assoc()['count'];
        ...
        // Validate that at least one debit entry was processed
        if ($valid_debit_entries == 0) {
            throw new Exception("No valid debit entries found. Please ensure at least one account and amount is entered.");
        }
        ...
        // Validate that credit account is not the same as any debit account
        foreach ($_POST['debit_account_id'] as $index => $debit_account) {
            if (!empty($debit_account) && $debit_account == $credit_account_id) {
                throw new Exception("Credit account cannot be the same as debit account: " . $debit_account);
            }
        }
        ...
        $_SESSION['error'] = "Failed to record payment: " . $e->getMessage() . ". Please verify all amounts and account selections are correct.";
    }
}
...
if ($branches->num_rows == 0) {
    $_SESSION['error'] = "No branches available. Please add branches first before creating account payments.";
}
        ...
        <div class="card-body">
            ...
            <?php if ($branches->num_rows == 0): ?>
                <div class="alert alert-warning">
                    ...
                    <strong>Warning:</strong> No branches are available. Please add branches first before creating account payments.
                </div>
            <?php else: ?>
